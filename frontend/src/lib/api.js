...
async function requestBlob(path, { method = 'GET', body } = {}) {
  ...
  const res = await fetch(`${API_URL}${path}`, { method, headers, body: body !== undefined ? JSON.stringify(body) : undefined });
  if (!res.ok) { let data = {}; try { data = await res.json(); } catch (e) {} throw new Error(data.error || `Request failed (${res.status})`); }
  return res.blob();
}

async function requestAllDateFilteredScreenshots({ employeeId, date }) {
  const all = []; const batchSize = 200; let offset = 0;
  while (true) {
...
export const api = {
  ...
  uploads: {
    upload: async (file) => {
      ...
      if (!res.ok) throw new Error(data.error || `Upload failed (${res.status})`); return data;
    },
  ...
  activity: {
    ...
    screenshotsPage: ({ page = 1, pageSize = 60, deviceId, date } = {}) => { const params = new URLSearchParams(); params.set('page', page); params.set('pageSize', pageSize); if (deviceId) params.set('deviceId', deviceId); if (date) params.set('date', date); return request(`/activity/screenshots-page?${params.toString()}`); },
