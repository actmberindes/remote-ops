...
export const agentMonitoringRouter = Router();
...
agentMonitoringRouter.get('/devices/:id/screenshots', ...monitoringAccess, (req, res) => {
  ...
  let list = (db.data.screenshots || [])
    ...
    .map(item => ({ ...item, domainUser: domainUserOf(item, device) }));
...
agentMonitoringRouter.get('/devices/:id/screenshot-users', ...monitoringAccess, (req, res) => {
  ...
  const users = [...new Set((db.data.screenshots || [])
    .filter(item => Number(item.deviceId) === Number(device.id))
    .map(item => domainUserOf(item, device))
