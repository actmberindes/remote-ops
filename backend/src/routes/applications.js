...
applicationsRouter.post('/', requireRole('Employee'), async (req, res) => {
  const { location, startDate, temporary, endDate, defaultEndDate, days, internetType, fileName, fileUrl, reason } = req.body || {};
  ...
  const app = {
    ...
    days, internetType, fileName: fileName || '', fileUrl: fileUrl || null, reason: reason || '',
  };
