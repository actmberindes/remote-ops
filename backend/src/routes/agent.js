...
function generateEnrollmentCode() { return crypto.randomInt(10000000, 100000000).toString(); }
function userName(id) { const u = db.data.users.find(x => x.id === id); return u ? u.name : 'Unknown'; }
...
agentRouter.post('/devices/register', requireAuth(db), requireRole('Admin'), async (req, res) => {
  ...
  if (!['desktop-agent', 'browser-extension'].includes(deviceType)) return res.status(400).json({ error: 'deviceType must be desktop-agent or browser-extension.' });
  const device = {
    ...
    ipAddress: null, operatingSystem: null, currentEmployeeId: null, currentSessionStartedAt: null, isRdp: false, sessionName: null, lastSeenAt: null, lastStateChangedAt: null, state: 'pending', enrolled: false, revoked: false,
  };
...
agentRouter.post('/enroll', async (req, res) => {
  ...
  device.currentEmployeeId = resolvedEmployee?.id || device.employeeId; device.currentSessionStartedAt = now; device.isRdp = Boolean(isRdp); device.sessionName = sessionName ? String(sessionName) : null; device.agentVersion = agentVersion ? String(agentVersion) : null; device.lastSeenAt = now;
...
agentRouter.post('/pairing-code', requireAuth(db), requireRole('Admin'), async (req, res) => res.status(410).json({ error: 'Employee pairing has been retired. Admins must register devices from Device Management.' }));
agentRouter.post('/pair', (req, res) => res.status(410).json({ error: 'Employee pairing has been retired. Use an Admin-generated device enrollment code.' }));
...
agentRouter.get('/session-status', requireDevice(db), (req, res) => {
  ...
  res.json({ status: resolveDeviceState(req.device), employeeName: current?.name || userName(req.device.employeeId), currentEmployeeId: current?.id || null, domainUser: req.device.domainUser || null, deviceName: req.device.deviceName, isRdp: req.device.isRdp === true, connectionType: req.device.isRdp ? 'RDP' : (req.device.domainUser ? 'Local' : null), sessionName: req.device.sessionName || null });
});

agentRouter.post('/heartbeat', requireDevice(db), async (req, res) => {
  const { state = 'active', hostname, machineId, domain, domainUser, isRdp, sessionName, agentVersion, ipAddress, operatingSystem } = req.body || {};
  const allowedStates = new Set(['active', 'idle', 'logged-out']); const nextState = allowedStates.has(state) ? state : 'active'; const now = new Date().toISOString();
  ...
  if (isRdp !== undefined) req.device.isRdp = Boolean(isRdp); if (sessionName !== undefined) req.device.sessionName = sessionName ? String(sessionName) : null; if (agentVersion) req.device.agentVersion = String(agentVersion);
  ...
  res.json({ ok: true, status: resolveDeviceState(req.device), currentEmployeeId: req.device.currentEmployeeId, currentEmployeeName: req.device.currentEmployeeId ? userName(req.device.currentEmployeeId) : null, domainUser: req.device.domainUser || null, isRdp: req.device.isRdp === true, connectionType: req.device.isRdp ? 'RDP' : (req.device.domainUser ? 'Local' : null), sessionName: req.device.sessionName || null, serverTime: now });
});

agentRouter.post('/quit-authorize', requireDevice(db), async (req, res) => {
  const configured = String(process.env.AGENT_ADMIN_QUIT_CODE || '').trim(); const supplied = String(req.body?.code || '').trim();
  if (!configured) return res.status(503).json({ error: 'Administrator quit code is not configured on the server.' }); if (!supplied || supplied.length > 128) return res.status(400).json({ error: 'Administrator code is required.' });
  const sameLength = configured.length === supplied.length; const matches = sameLength && crypto.timingSafeEqual(Buffer.from(configured), Buffer.from(supplied)); if (!matches) return res.status(403).json({ error: 'Invalid administrator code.' });
  const now = new Date().toISOString(); req.device.currentEmployeeId = null; req.device.currentSessionStartedAt = null; recordStateChange(req.device, 'offline', now); req.device.lastSeenAt = now; await db.write();
...
agentRouter.put('/config', requireAuth(db), requireRole('Admin'), async (req, res) => {
  ...
  if (screenshotIntervalMinutes !== undefined) { if (!VALID_INTERVALS.includes(Number(screenshotIntervalMinutes))) return res.status(400).json({ error: `screenshotIntervalMinutes must be one of ${VALID_INTERVALS.join(', ')}.` }); db.data.agentConfig.screenshotIntervalMinutes = Number(screenshotIntervalMinutes); }
  if (liveViewFrameIntervalSeconds !== undefined) { const v = Number(liveViewFrameIntervalSeconds); if (!(v >= 2 && v <= 60)) return res.status(400).json({ error: 'liveViewFrameIntervalSeconds must be between 2 and 60.' }); db.data.agentConfig.liveViewFrameIntervalSeconds = v; }
  if (screenshotRetentionDays !== undefined) { const v = Number(screenshotRetentionDays); if (!(v >= 1 && v <= 365)) return res.status(400).json({ error: 'screenshotRetentionDays must be between 1 and 365.' }); db.data.agentConfig.screenshotRetentionDays = v; }
  if (liveViewRetentionDays !== undefined) { const v = Number(liveViewRetentionDays); if (!(v >= 1 && v <= 365)) return res.status(400).json({ error: 'liveViewRetentionDays must be between 1 and 365.' }); db.data.agentConfig.liveViewRetentionDays = v; }
  if (webUsageRetentionDays !== undefined) { const v = Number(webUsageRetentionDays); if (!(v >= 1 && v <= 365)) return res.status(400).json({ error: 'webUsageRetentionDays must be between 1 and 365.' }); db.data.agentConfig.webUsageRetentionDays = v; }
...
agentRouter.get('/devices/:id/history', requireAuth(db), requireRole('Admin'), (req, res) => {
  const id = Number(req.params.id); const device = db.data.devices.find(d => d.id === id); if (!device) return res.status(404).json({ error: 'Device not found.' });
  const history = (db.data.deviceStateHistory || []).filter(h => h.deviceId === id).sort((a, b) => (a.timestamp < b.timestamp ? 1 : -1)).slice(0, 500); res.json({ device: publicDevice(device), history });
});
agentRouter.get('/devices/:id/details', requireAuth(db), requireRole('Admin'), (req, res) => {
  const id = Number(req.params.id); const device = db.data.devices.find(d => d.id === id); if (!device) return res.status(404).json({ error: 'Device not found.' });
  const history = (db.data.deviceStateHistory || []).filter(item => Number(item.deviceId) === id).sort((a, b) => new Date(b.timestamp).getTime() - new Date(a.timestamp).getTime()).slice(0, 5000).map(item => ({ ...item, domainUser: item.domainUser || null }));
  const screenshots = (db.data.screenshots || []).filter(item => Number(item.deviceId) === id).sort((a, b) => new Date(b.capturedAt).getTime() - new Date(a.capturedAt).getTime()).slice(0, 1000).map(item => ({ ...item, domainUser: item.domainUser || device.domainUser || null }));
  const liveFrames = (db.data.liveFrameHistory || []).filter(item => Number(item.deviceId) === id).sort((a, b) => new Date(b.capturedAt).getTime() - new Date(a.capturedAt).getTime()).slice(0, 500).map(item => ({ ...item, domainUser: item.domainUser || device.domainUser || null }));
...
agentRouter.patch('/devices/:id/revoke', requireAuth(db), requireRole('Admin'), async (req, res) => {
  const id = Number(req.params.id); const device = db.data.devices.find(d => d.id === id); if (!device) return res.status(404).json({ error: 'Device not found.' });
  const now = new Date().toISOString(); device.revoked = true; device.currentEmployeeId = null; recordStateChange(device, 'revoked', now); await db.write(); res.json(publicDevice(device));
});
agentRouter.delete('/devices/:id', requireAuth(db), requireRole('Admin'), async (req, res) => {
  const id = Number(req.params.id); const index = db.data.devices.findIndex(d => d.id === id); if (index === -1) return res.status(404).json({ error: 'Device not found.' });
  db.data.deviceStateHistory = (db.data.deviceStateHistory || []).filter(h => h.deviceId !== id); db.data.devices.splice(index, 1); await db.write(); res.json({ ok: true, deviceId: id });
});
