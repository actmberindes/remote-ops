...
function promptAdminCodeInteractive() {
  return new Promise(resolve => {
    ...
    const child = spawn(
      'powershell.exe',
      ['-NoProfile', '-STA', '-WindowStyle', 'Hidden', '-ExecutionPolicy', 'Bypass', '-Command', script],
      {
        windowsHide: true,
    ...
    child.on('error', err => {
      console.error(`Administrator prompt failed to start: ${err.message}`);
      resolve(null);
    });
    child.on('close', code => {
      if (code !== 0 && stderr.trim()) console.error(`Administrator prompt error: ${stderr.trim()}`);
...
function startTray({ employeeName, deviceToken, client, onQuit } = {}) {
  if (process.platform !== 'win32') {
    return { ready: Promise.resolve(), setStatus() {}, async kill() {} };
  }
